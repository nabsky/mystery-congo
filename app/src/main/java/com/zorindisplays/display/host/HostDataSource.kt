...
class HostDataSource(
    ...
    override val events: SharedFlow<DemoEvent> get() = _events

    private var systemMode: DemoState.SystemMode = DemoState.SystemMode.ACCEPTING_BETS
        ...
        for (boxId in boxIds) {
            for (jackpotId in listOf("RUBY", "GOLD", "JADE")) {
                ...
                if (win == null && Random.nextInt(n) == 0) {
                    ...
                    pendingWin = win
                    _events.tryEmit(DemoEvent.JackpotHit(jackpotId, tableId, boxId, win.winAmount))
    ...
    private fun PendingWin.toDemoPendingWin(): DemoState.PendingWin =
        DemoState.PendingWin(jackpotId, tableId, boxId, winAmount)
